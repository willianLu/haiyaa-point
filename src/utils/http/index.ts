...
export function request<T, D, U>(
...
): Promise<[RequestBackData<T, D, U> | undefined, any | undefined]> {
  const { method, isFormData } = config
  let data: any = config.data || {}
  if (method === 'get') {
    data = config.params || {}
  }
  data.token = session.getItem('token')
  data.userId = session.getItem('userId')
  if (isFormData) {
  ...
  return new Promise<any>(resolve => {
    HttpRequest.request<T, AxiosResponse<CustomResponseData, D>, D>(config)
      .then(res => {
...
export async function get<T = any, D = any, U = 'data'>(
  url: string,
  params?: D,
  config?: CustomAxiosRequestConfig<D>
) {
 ...
 * @returns {Promise<any>}
 */
export function post<T = any, D = any, U = 'data'>(
  url: string,
  data?: D,
  config?: CustomAxiosRequestConfig<D>
) {
